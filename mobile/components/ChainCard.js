...
export default function ChainCard({
  ...
  async function report() {
    await onReport?.(entry, { reason, details });
    setReportOpen(false);
    setDetails("");
  }
    ...
    router.push({
      ...
      params: {
        ...
        startParagraph: String(
          ...
          (
            Number(entry.paragraphNumber) > 0
              ? Number(entry.paragraphNumber) - 1
              : 0
          )
        )
      }
  ...
  return (
    <View style={styles.card}>
      <View style={styles.topRow}>
        ...
        {!!entry.bookId && (
          <Pressable onPress={openBook} style={styles.bookButton}>
      ...
      <View style={styles.secondary}>
        <Pressable onPress={save} style={styles.secondaryButton}>
          <Text style={styles.secondaryGlyph}>
            {saved ? "◆" : "◇"}
          </Text>
      ...
      {showComposer && (
        <>
          ...
          <Pressable
            disabled={adding || !String(linkText || "").trim()}
            onPress={() => onAddLink?.(entry)}
            style={[
              styles.addButton,
              (
                adding ||
        ...
        <View style={styles.modalShade}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.reasonRow}>
              {["spam", "harassment", "copyright", "other"].map((value) => (
                <Pressable
                  key={value}
                  onPress={() => setReason(value)}
                  style={[
                    styles.reasonChip,
                    reason === value && styles.reasonChipActive
                  ]}
                >
            ...
            <Pressable onPress={report} style={styles.reportSubmit}>
