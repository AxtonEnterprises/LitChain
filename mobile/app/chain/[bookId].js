...
export default function ChainBookScreen() {
  ...
  async function fetchChainData() {
    const [filteredFeed, publicFeed] =
      await Promise.all([
        getChainFeedByFilter(filter),
        getPublicChainFeed()
      ]);

    /*
     * Level 1 respects the selected filter.
     * Public branch children are also loaded so a newly-added
     * public link remains available after a refresh even when
     * the Level-0 filter is Friends.
     */
    const merged =
      mergeEntries(
        filteredFeed,
  ...
  async function addLink(entry) {
    ...
    try {
      ...
      /*
       * Do not rely on a local-only append. Reload Firestore
       * immediately and verify the child is actually present
       * in the persisted public Chain.
       */
      const {
        merged
      } =
      ...
      if (!persisted) {
        throw new Error(
          "The link was saved, but could not be reloaded from the Chain."
        );
      }
