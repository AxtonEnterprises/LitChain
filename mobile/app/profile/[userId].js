    ...
    <ScrollView contentContainerStyle={s.scroll}>
      <View style={s.profileCard}>
        ...
        <View style={s.profileBody}>
          ...
          <View style={s.stats}>
            <Pressable accessibilityRole="button" accessibilityLabel={`${Number(stats.books || 0)} books in reading history`} onPress={() => { const count = Number(stats.books || 0); Alert.alert("Reading History", `${count} ${count === 1 ? "book" : "books"} in this reader's public reading timeline.`); }} style={s.statPill}><Text style={s.statIcon}>📖</Text><Text style={s.statValue}>{Number(stats.books || 0)}</Text></Pressable>
            <Pressable accessibilityRole="button" accessibilityLabel={`${Number(stats.completed || 0)} completed books`} onPress={() => { const count = Number(stats.completed || 0); Alert.alert("Completed Books", `${count} ${count === 1 ? "book" : "books"} completed.`); }} style={s.statPill}><Text style={s.statIcon}>📕</Text><Text style={s.statValue}>{Number(stats.completed || 0)}</Text></Pressable>
            <Pressable accessibilityRole="button" accessibilityLabel={`${Number(stats.journal || 0)} journal entries`} onPress={() => { const count = Number(stats.journal || 0); Alert.alert("Journal Entries", `${count} ${count === 1 ? "entry" : "entries"} in this reader's journal.`); }} style={s.statPill}><Text style={s.statIcon}>📓</Text><Text style={s.statValue}>{Number(stats.journal || 0)}</Text></Pressable>
          </View>
