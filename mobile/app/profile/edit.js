...
export default function EditProfileScreen() {
  ...
  function confirmDeletion(){
    Alert.alert(
      ...
      "This requests deletion of your Lit Chain account and associated personal data. Public community content may be retained only where necessary for conversation continuity and will be anonymized during processing. You will be signed out after the request is submitted.",
      [
  ...
  async function submitDeletionRequest(){
    try{
      ...
      setStatus(error?.message||"Account deletion could not be requested.");
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.form}>
        ...
        <FlatList horizontal data={PROFILE_AVATARS} keyExtractor={item=>item.id} showsHorizontalScrollIndicator={false}
          renderItem={({item})=>(
            <Pressable onPress={()=>setAvatar(item.id)} style={[styles.avatarWrap,avatar===item.id&&styles.avatarSelected]}>
...
const styles=StyleSheet.create({
  ...
  avatarWrap:{width:72,height:72,borderRadius:16,marginRight:10,padding:3},
  ...
  deleteText:{color:BRAND.danger,fontWeight:"900"},
  disabled:{opacity:.55}
});
